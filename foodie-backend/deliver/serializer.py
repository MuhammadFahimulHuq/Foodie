...
class DeliverSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = Deliver
        fields = ['username', 'password', 'email','phoneNo', 'first_name', 'last_name', 'address', 'dateOfBirth','nidNo']


    def validate_email(self, value):
        norm_email = value.lower()
        if Deliver.objects.filter(email=norm_email).exists():
            raise serializers.ValidationError("Not unique email")
        return norm_email

    def validate_username(self, value):
        norm_username = value.lower()
        if Deliver.objects.filter(username=norm_username).exists():
    ...
    def validate_phoneNo(self, value):
        ...
        if Deliver.objects.filter(phoneNo=norm_phoneNo).exists():
            raise serializers.ValidationError("Not unique phone number")
        return norm_phoneNo

    def validate_nidNo(self, value):
        norm_nidNo = value.lower()
        if Deliver.objects.filter(nidNo=norm_nidNo).exists():
            raise serializers.ValidationError("Not unique national identification number")
        return norm_nidNo

...
class CustomerSerializer(serializers.ModelSerializer):
    ...
    def validate_email(self, value):
        ...
        if Customer.objects.filter(email=norm_email).exists():
            raise serializers.ValidationError("Not unique email")
        return norm_email

    def validate_username(self, value):
        norm_username = value.lower()
        if Customer.objects.filter(username=norm_username).exists():
    ...
    def validate_phoneNo(self, value):
        ...
        if Customer.objects.filter(phoneNo=norm_phoneNo).exists():
